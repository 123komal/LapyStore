...
var SekindoClientDetections_URL = function (config) {
	...
	this.placemenType = config.placemenType;
	...
	this.process = function()
	{
		...
		if (this.frameInfo.isBuildFrame)
		{
			...
			if (this.frameInfo.isBuildFrameViaJs)
			{
				...
				{
					iframe.style.width = ref.config.unit.width;
					iframe.style.height = ref.config.unit.height;
				}
				else
				{
	...
	{
		if (this.placemenType !== 'slBanner' && this.placemenType !== 'outstream' && this.placemenType !== 'vast')
		{
			return 'banner';
		}
		return 'video';
	...
	this.setInfo();
				...
				var urlDetObj = new SekindoClientDetections_URL({
					url: 'https://live.sekindo.com/live/liveView.php?s=105117&cbuster=[CACHE_BUSTER]&pubUrl=[PAGE_URL_ENCODED]&subId=[SUBID_ENCODED]&vp_content=plembed1817kuirwnoj&cbuster=1612882480',
					origQString: 's=105117&cbuster=[CACHE_BUSTER]&pubUrl=[PAGE_URL_ENCODED]&subId=[SUBID_ENCODED]&vp_content=plembed1817kuirwnoj',
					placementId: 105117,
					x: 300,
					y: 600,
					videoType: 'normal',
